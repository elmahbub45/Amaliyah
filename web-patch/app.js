...
function prayerTimingAccess(){
  if(!nativeNotificationApp())return 'not-needed';
  try{
    return typeof window.AmaliyahAndroid.getPrayerTimingAccess==='function'
      ? String(window.AmaliyahAndroid.getPrayerTimingAccess()||'needed')
      : 'needed';
  }catch{return 'needed'}
}
function updatePrayerTimingUI(){
  const box=$('#notificationTimingAccess');
  if(!box)return;
  if(!nativeNotificationApp()){box.classList.add('hidden');return;}
  box.classList.remove('hidden');
  const access=prayerTimingAccess();
  const ready=access==='granted' || access==='not-needed';
  box.classList.toggle('is-ready',ready);
  const text=$('#notificationTimingText');
  const btn=$('#notificationTimingBtn');
  if(ready){
    if(text)text.textContent='Amaliyah siap menjalankan pengingat pada waktu yang dijadwalkan.';
    if(btn){btn.textContent='Sudah Diizinkan ✓';btn.disabled=true;}
  }else{
    if(text)text.textContent='Izinkan Amaliyah agar pengingat dapat berbunyi lebih tepat waktu saat aplikasi ditutup.';
    if(btn){btn.textContent='Izinkan';btn.disabled=false;}
  }
}
async function requestPrayerTimingAccess(){
  if(!nativeNotificationApp())return;
  try{window.AmaliyahAndroid.requestPrayerTimingAccess?.();}
  catch{}
  setTimeout(updatePrayerTimingUI,500);
}
...
async function testPushNotification(){
  ...
  try{
    ...
    if(nativeNotificationApp()){
      const mode=prayerSoundMode();
      if(typeof window.AmaliyahAndroid.testPrayerNotification==='function'){
        window.AmaliyahAndroid.testPrayerNotification(mode,'Dzuhur');
      }else{
        await showAppNotification('Tes Amaliyah','Tes notifikasi berhasil.');
      }
      await appNotice(mode==='adhan'?'Tes adzan telah dijalankan.':mode==='short'?'Tes pengingat singkat telah dijalankan.':'Tes notifikasi telah dikirim.');
      return;
    }
...
function getNotificationSettings(){
  try{
    ...
    if(!['notification','short','adhan'].includes(settings.sound))settings.sound='notification';
    if(settings.sound==='adhan')settings.lead=0;
...
function buildNativePrayerPayload(){
  const s=getNotificationSettings();
  const loc=getSavedLocation();
  return {
    date:dateKey(),
    timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||'Asia/Makassar',
    enabled:!!s.enabled,
    leadMinutes:s.sound==='adhan'?0:Number(s.lead||0),
    soundMode:s.sound||'notification',
    prayers:{...s.prayers},
    schedule:buildPushSchedule(),
    location:loc?{
      latitude:Number(loc.latitude),longitude:Number(loc.longitude),label:loc.label||'',
      regionName:loc.regionName||'',regionId:loc.prayerRegionId||'',
...
function syncNativePrayerSchedule(){
  if(!nativeNotificationApp())return;
  try{
    const payload=buildNativePrayerPayload();
    if(!payload.enabled){window.AmaliyahAndroid.cancelPrayerAlarms?.();return;}
    if(!latestPrayerTimes || !payload.location)return;
    window.AmaliyahAndroid.syncPrayerSchedule?.(JSON.stringify(payload));
  }catch{}
}
function prayerSoundMode(){return getNotificationSettings().sound||'notification';}
function updatePrayerSoundUI(){
  const s=getNotificationSettings();
  const native=nativeNotificationApp();
  document.querySelectorAll('.native-prayer-sound').forEach(node=>node.classList.toggle('hidden',!native));
  if(!native && s.sound!=='notification'){
    s.sound='notification';
    setNotificationSettings(s);
  }
  document.querySelectorAll('input[name="notificationSound"]').forEach(input=>input.checked=input.value===s.sound);
  const lead=$('#notificationLead');
  const leadWrap=lead?.closest('.notification-reminder');
  const hint=$('#notificationSoundHint');
  if(s.sound==='adhan'){
    if(lead){lead.value='0';lead.disabled=true;}
    leadWrap?.classList.add('is-locked');
    if(hint)hint.textContent='Adzan lengkap selalu berbunyi tepat saat masuk waktu sholat.';
  }else{
    if(lead)lead.disabled=false;
    leadWrap?.classList.remove('is-locked');
    if(hint)hint.textContent=s.sound==='short'
      ? 'Pengingat singkat dapat digunakan tepat waktu atau beberapa menit sebelumnya.'
      : 'Menggunakan bunyi notifikasi perangkat.';
  }
}
function stopPrayerPreview(){
  if(prayerPreviewAudio){try{prayerPreviewAudio.pause();prayerPreviewAudio.currentTime=0;}catch{} prayerPreviewAudio=null;}
  try{window.AmaliyahAndroid?.stopPrayerSoundPreview?.();}catch{}
  const btn=$('#notificationSoundPreview'); if(btn)btn.textContent='Dengarkan Contoh';
}
async function previewPrayerSound(){
  const mode=prayerSoundMode();
  const btn=$('#notificationSoundPreview');
  if(btn?.dataset.playing==='1'){
    btn.dataset.playing='0';stopPrayerPreview();return;
  }
  ...
  stopPrayerPreview();
  if(btn){btn.dataset.playing='1';btn.textContent='Hentikan';}
  if(nativeNotificationApp() && typeof window.AmaliyahAndroid.previewPrayerSound==='function'){
    try{
      window.AmaliyahAndroid.previewPrayerSound(mode,'Dzuhur');
      const previewMs=mode==='short'?12500:195000;
      setTimeout(()=>{
        if(btn?.dataset.playing==='1'){btn.dataset.playing='0';btn.textContent='Dengarkan Contoh';}
      },previewMs);
      return;
    }catch{}
  }
  const src=mode==='short'?'./assets/audio/adzan-pendek.mp3':'./assets/audio/adzan.mp3';
  prayerPreviewAudio=new Audio(src);
  prayerPreviewAudio.onended=()=>{if(btn){btn.dataset.playing='0';btn.textContent='Dengarkan Contoh';}prayerPreviewAudio=null;};
  prayerPreviewAudio.onerror=()=>{if(btn){btn.dataset.playing='0';btn.textContent='Dengarkan Contoh';}prayerPreviewAudio=null;appNotice('Contoh suara belum dapat diputar.');};
  try{await prayerPreviewAudio.play();}catch{if(btn){btn.dataset.playing='0';btn.textContent='Dengarkan Contoh';}}
}
...
function saveNotificationSettings(){
  const s=getNotificationSettings();
  const chosen=document.querySelector('input[name="notificationSound"]:checked')?.value;
  if(['notification','short','adhan'].includes(chosen))s.sound=chosen;
  s.lead=s.sound==='adhan'?0:+($('#notificationLead')?.value||0);
  document.querySelectorAll('[data-prayer-notif]').forEach(box=>{
...
function checkPrayerNotifications(){
  if(nativeNotificationApp())return; // APK menjadwalkan pengingat secara native agar tetap berjalan saat aplikasi ditutup.
  const s=getNotificationSettings();
...
window.previewPrayerSound=previewPrayerSound;
window.requestPrayerTimingAccess=requestPrayerTimingAccess;
window.addEventListener('amaliyah-prayer-timing-access-changed',()=>{updatePrayerTimingUI();syncNativePrayerSchedule();});
